...
void setCudaDeviceFromRank(miniFE::Parameters &params) {
  ...
  if(params.num_devices<0)
    params.num_devices = dpct::dev_mgr::instance().device_count();
  ...
  dpct::dev_mgr::instance().select_device(params.device);
}
...
int main(int argc, char** argv) {
  ...
  dpct::dev_mgr::instance().get_device(dev).get_device_info(info);
