...
struct cuda_map_ids_to_rows {
  ...
  dpct::device_vector<GlobalOrdinal> d_ids;
  ...
  void set_map(const std::map<GlobalOrdinal,GlobalOrdinal> &map) {
    ...
    dpct::async_dpct_memcpy(dpct::get_raw_pointer(&d_ids[0]), &ids[0],
                            size * sizeof(GlobalOrdinal), dpct::host_to_device,
                            *CudaManager::s1);
    dpct::async_dpct_memcpy(dpct::get_raw_pointer(&d_rows[0]), &rows[0],
                            size * sizeof(GlobalOrdinal), dpct::host_to_device,
                            *CudaManager::s1);
  }
  PODMap<GlobalOrdinal> getPOD() const {
    ...
    ret.n=d_ids.size();
    ret.rows = const_cast<GlobalOrdinal *>(dpct::get_raw_pointer(&d_rows[0]));
    ret.ids = const_cast<GlobalOrdinal *>(dpct::get_raw_pointer(&d_ids[0]));
    return ret;
  }
};
