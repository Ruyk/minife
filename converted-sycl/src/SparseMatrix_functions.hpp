...
namespace miniFE {
   ...
   ****************************************************************/
  template <class GlobalOrdinal>
    int inline pitchedBinarySearch(GlobalOrdinal *indices, GlobalOrdinal low, GlobalOrdinal high, GlobalOrdinal _val, const GlobalOrdinal pitch)
    {
      ...
      while(high>=low)
      {
        ...
        /*
        DPCT1026:31: The call to __ldg was removed because there is no
...
#pragma unroll
  ...
  template<typename MatrixType, typename VectorType>
    ...
    impose_dirichlet_first_kernel(typename MatrixType::ScalarType prescribed_value,
      ...
      for (int i = item_ct1.get_group(2) * item_ct1.get_local_range().get(2) +
           ...
           i < num_bc_rows; i += item_ct1.get_local_range().get(2) *
  ...
  template<typename MatrixType, typename VectorType>
    ...
    impose_dirichlet_second_kernel(typename MatrixType::ScalarType prescribed_value,
      ...
      for (int i = item_ct1.get_group(2) * item_ct1.get_local_range().get(2) +
                   item_ct1.get_local_id(2);
           i < A.num_rows; i += item_ct1.get_local_range().get(2) *
                                item_ct1.get_group_range(2)) {
  ...
  template<typename MatrixType,
    typename VectorType>
          ...
          const std::set<typename MatrixType::GlobalOrdinalType>& bc_rows)
      {
        ...
        if(num_bc_rows>0) {
          ...
          dpct::device_vector<GlobalOrdinal> d_bc_rows_vec(num_bc_rows);
          dpct::async_dpct_memcpy(dpct::get_raw_pointer(&d_bc_rows_vec[0]),
                                  &bc_rows_vec[0],
                                  sizeof(GlobalOrdinal) * num_bc_rows,
          ...
          NUM_BLOCKS = std::min(
              MAX_BLOCKS, (int)(num_bc_rows + BLOCK_SIZE - 1) / BLOCK_SIZE);
          //TODO place in different streams...
          /*
          DPCT1049:32: The workgroup size passed to the SYCL kernel may exceed
    ...
    CudaManager::s1->submit([&](sycl::handler &cgh) {
      ...
      auto b_getPOD_ct2 = b.getPOD();
      auto thrust_raw_pointer_cast_d_bc_rows_vec_ct3 =
          dpct::get_raw_pointer(&d_bc_rows_vec[0]);

      cgh.parallel_for<
          dpct_kernel_name<class impose_dirichlet_first_kernel_e36e54,
          ...
          sycl::nd_range<3>(sycl::range<3>(1, 1, NUM_BLOCKS) *
                            ...
                            sycl::range<3>(1, 1, BLOCK_SIZE)),
          [=](sycl::nd_item<3> item_ct1) {
            impose_dirichlet_first_kernel(
                prescribed_value, A_getPOD_ct1, b_getPOD_ct2,
                thrust_raw_pointer_cast_d_bc_rows_vec_ct3, num_bc_rows,
                first_local_row, last_local_row, item_ct1);
          });
    });
          cudaCheckError();
          NUM_BLOCKS = std::min(MAX_BLOCKS,
                                (int)(num_rows + BLOCK_SIZE - 1) / BLOCK_SIZE);
          /*
          DPCT1049:33: The workgroup size passed to the SYCL kernel may exceed
          the limit. To get the device limit, query
          info::device::max_work_group_size. Adjust the workgroup size if
          needed.
          */
    CudaManager::s1->submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for<
          dpct_kernel_name<class impose_dirichlet_second_kernel_d5ac53,
                           MatrixType /*Fix the type mannually*/,
                           VectorType /*Fix the type mannually*/>>(
          sycl::nd_range<3>(sycl::range<3>(1, 1, NUM_BLOCKS) *
                                sycl::range<3>(1, 1, BLOCK_SIZE),
                            sycl::range<3>(1, 1, BLOCK_SIZE)),
          [=](sycl::nd_item<3> item_ct1) {
            impose_dirichlet_second_kernel(
                prescribed_value, A_getPOD_ct1, b_getPOD_ct2,
                thrust_raw_pointer_cast_d_bc_rows_vec_ct3, num_bc_rows,
                item_ct1);
          });
  ...
  __inline__ MATVEC_RANGE get_range(int mask, int bit) {
      return static_cast<MATVEC_RANGE>( (mask>>bit)&1);
  }

  template<typename MatrixType>
      void createExternalMapping(MatrixType A, sycl::nd_item<3> item_ct1) {
        typedef typename MatrixType::GlobalOrdinalType GlobalOrdinalType;
        for (int row_idx =
  ...
  template<typename MatrixType,
    typename VectorType>
      void matvec_ell_kernel(MatrixType A, VectorType X, VectorType Y,
                             sycl::nd_item<3> item_ct1) {
        typedef typename MatrixType::ScalarType ScalarType;
        typedef typename MatrixType::GlobalOrdinalType GlobalOrdinalType;
  ...
  template<MATVEC_RANGE RANGE, typename MatrixType,
    typename VectorType>
      void matvec_overlap_ell_kernel(MatrixType A, VectorType X, VectorType Y,
                                     sycl::nd_item<3> item_ct1) {
        typedef typename MatrixType::ScalarType ScalarType;
        typedef typename MatrixType::GlobalOrdinalType GlobalOrdinalType;
          ...
          for(int j=0;j<A.num_cols_per_row;++j)
          {
            if(RANGE == get_range(bitfield,j)) {
              ...
              if(c!=-1) {
                ScalarType a=A.coefs[offset];
                /*
                DPCT1026:34: The call to __ldg was removed because there is no
                correspoinding API in DPC++.
                */
                ScalarType x = *(X.coefs + c);
                sum+=a*x;
              }
            }
            offset+=A.pitch;
  ...
  template<typename MatrixType,
    ...
    CudaManager::s1->submit([&](sycl::handler &cgh) {
      ...
      auto y_getPOD_ct2 = y.getPOD();

      cgh.parallel_for<dpct_kernel_name<
          class matvec_overlap_ell_kernel_ab010d, dpct_kernel_scalar<INTERNAL>,
          MatrixType /*Fix the type mannually*/,
          VectorType /*Fix the type mannually*/>>(
          sycl::nd_range<3>(sycl::range<3>(1, 1, NUM_BLOCKS) *
                                sycl::range<3>(1, 1, BLOCK_SIZE),
                            sycl::range<3>(1, 1, BLOCK_SIZE)),
          [=](sycl::nd_item<3> item_ct1) {
            matvec_overlap_ell_kernel<INTERNAL>(A_getPOD_ct0, x_getPOD_ct1,
                                                y_getPOD_ct2, item_ct1);
          });
    });
          nvtxRangeEnd(r2);
    ...
    CudaManager::s1->submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for<dpct_kernel_name<
          class matvec_overlap_ell_kernel_b1fa50, dpct_kernel_scalar<EXTERNAL>,
          MatrixType /*Fix the type mannually*/,
          VectorType /*Fix the type mannually*/>>(
          sycl::nd_range<3>(sycl::range<3>(1, 1, NUM_BLOCKS) *
                                sycl::range<3>(1, 1, BLOCK_SIZE),
                            sycl::range<3>(1, 1, BLOCK_SIZE)),
          [=](sycl::nd_item<3> item_ct1) {
            matvec_overlap_ell_kernel<EXTERNAL>(A_getPOD_ct0, x_getPOD_ct1,
                                                y_getPOD_ct2, item_ct1);
          });
    });
          nvtxRangeEnd(r4);
