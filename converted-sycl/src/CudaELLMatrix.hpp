...
CudaELLMatrix {
  ...
  {
    /*
    DPCT1026:8: The call to cudaHostUnregister was removed because DPC++
  ...
  void copyToDevice() const {
    int n=d_coefs.size();
    dpct::dpct_memcpy(const_cast<Scalar *>(dpct::get_raw_pointer(&d_coefs[0])),
                      const_cast<Scalar *>(&coefs[0]), sizeof(Scalar) * n,
                      dpct::host_to_device);
    cudaCheckError();
    dpct::dpct_memcpy(
        const_cast<GlobalOrdinal *>(dpct::get_raw_pointer(&d_cols[0])),
        const_cast<GlobalOrdinal *>(&cols[0]), sizeof(GlobalOrdinal) * n,
        dpct::host_to_device);
    cudaCheckError();
    dpct::dpct_memcpy(
        const_cast<GlobalOrdinal *>(dpct::get_raw_pointer(&d_rows[0])),
        const_cast<GlobalOrdinal *>(&rows[0]),
        sizeof(GlobalOrdinal) * rows.size(), dpct::host_to_device);
    cudaCheckError();
  }
  void copyToHost() const {
    int n=d_coefs.size();
    dpct::dpct_memcpy(const_cast<Scalar *>(&coefs[0]),
                      const_cast<Scalar *>(dpct::get_raw_pointer(&d_coefs[0])),
                      sizeof(Scalar) * n, dpct::device_to_host);
    cudaCheckError();
    dpct::dpct_memcpy(
        const_cast<GlobalOrdinal *>(&cols[0]),
        const_cast<GlobalOrdinal *>(dpct::get_raw_pointer(&d_cols[0])),
        sizeof(GlobalOrdinal) * n, dpct::device_to_host);
    cudaCheckError();
    dpct::dpct_memcpy(
        const_cast<GlobalOrdinal *>(&rows[0]),
        const_cast<GlobalOrdinal *>(dpct::get_raw_pointer(&d_rows[0])),
        sizeof(GlobalOrdinal) * rows.size(), dpct::device_to_host);
    cudaCheckError();
  }

  PODELLMatrix<Scalar,LocalOrdinal,GlobalOrdinal> getPOD() {
    PODELLMatrix<Scalar,LocalOrdinal,GlobalOrdinal> ret;
    ret.rows = dpct::get_raw_pointer(&d_rows[0]);
    ret.cols = dpct::get_raw_pointer(&d_cols[0]);
    ret.coefs = dpct::get_raw_pointer(&d_coefs[0]);
#ifdef HAVE_MPI
    ret.external_map = dpct::get_raw_pointer(&d_external_map[0]);
#endif
  ...
  dpct::device_vector<GlobalOrdinal> d_rows;
  dpct::device_vector<GlobalOrdinal> d_cols;
  dpct::device_vector<Scalar> d_coefs;
  ...
  dpct::device_vector<GlobalOrdinal> d_elements_to_send;
  dpct::device_vector<int> d_external_map;
...
#endif
  dpct::device_vector<Scalar> d_send_buffer;
