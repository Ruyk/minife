                     ...
                     )
{
  ...
  d_elemIds = (GlobalOrdinal *)sycl::malloc_device(
      sizeof(GlobalOrdinal) * num_elems, dpct::get_default_queue());
  CudaManager::s1->memcpy(d_elemIds, &elemIDs[0],
  ...
  CudaManager::s1->memcpy(dpct::get_raw_pointer(&d_gradients[0]), &gradients[0],
                          sizeof(Scalar) * gradients.size());
  CudaManager::s1->memcpy(dpct::get_raw_pointer(&d_psi[0]), &psi[0],
                          sizeof(Scalar) * psi.size());

  //copy gauss_pts to constant memory
  CudaManager::s1->memcpy(Hex8::gauss_pts_c.get_ptr(*CudaManager::s1), gp,
                          sizeof(gp));
  cudaCheckError();
  ...
  */
  ...
  CudaManager::s1->submit([&](sycl::handler &cgh) {
    ...
    auto gauss_pts_c_ptr_ct1 = Hex8::gauss_pts_c.get_ptr();
    ...
    cgh.parallel_for<dpct_kernel_name<class element_loop_kernel_b38dfa,
        ...
        [=](sycl::nd_item<3> item_ct1) {
          element_loop_kernel(
              mesh_getPOD_ct0, num_elems, d_elemIds, A_getPOD_ct3, b_getPOD_ct4,
              thrust_raw_pointer_cast_d_gradients_ct5,
              thrust_raw_pointer_cast_d_psi_ct6, item_ct1, gauss_pts_c_ptr_ct1);
        });
  });
